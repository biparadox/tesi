...
main_v1_1( void )
{
	char		*function = "TESI_bind_unbinddata";
	TSS_HKEY	hKey;
	TSS_RESULT	result;
	UINT32		exitCode;
	...
	result=TESI_Local_ReadKeyBlob(&hKey,"bind/bindkey");
	if ( result != TSS_SUCCESS )
	{
		print_error( "Tspi_Key_LoadKey", result );
		exit( result );
	...
	result=TESI_Local_UnBind("bind/cipher.txt",hKey, "bind/plain1.txt");
	if ( result != TSS_SUCCESS )
	{
		print_error( "TESI_Local_Bind", result );
		exit( result );
	}
	TESI_Local_Fin();
	return;
