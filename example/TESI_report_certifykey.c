...
main_v1_1( void )
{
	char		*function = "TESI_bind_certifykey";
	TSS_HKEY	hKey;
	TSS_HKEY	*hPubKey;
	TSS_HKEY	hAIKey;
	TSS_RESULT	result;
	...
	result=TESI_Local_ReadKeyBlob(&hAIKey,"AIK");
	if ( result != TSS_SUCCESS )
	{
		print_error( "TESI_Local_ReadKeyBlob", result );
		exit( result );
	}
	...
	BYTE digest[20];	
	result=TESI_Local_ReadPubKey(&hPubKey,"bind/bindpubkey");
	if ( result != TSS_SUCCESS )
	{
		print_error( "TESI_Local_ReadPubKey", result );
		exit( result );
	}

	result=TESI_Report_GetKeyDigest(hPubKey,digest);
	if ( result != TSS_SUCCESS )
	{
		print_error( "TESI_Report_GetKeyDigest", result );
		exit( result );
	}
	...
	TESI_Report_CertifyKey(hKey,hAIKey,"valtest");	
	
	KEY_CERT * cert;

	cert=create_key_certify_struct("valtest",NULL,NULL);
